    ...
    return {
        ...
        onOk: function() {
            var b = CKEDITOR.tools.trim(this.getValueOf("info", "txtName")),
                ...
                a = this.getModel(c);
            a ? a.data("cke-realelement") ? (b = d(c, b), b.replace(a), CKEDITOR.env.ie && c.getSelection().selectElement(b)) : a.setAttributes(b) : (a = (a = c.getSelection()) && a.getRanges()[0], a.collapsed ? (b = d(c, b), a.insertNode(b)) : (CKEDITOR.env.ie && 9 > CKEDITOR.env.version && (b["class"] = "cke_anchor"), b = new CKEDITOR.style({
                    element: "a",
                    attributes: b
